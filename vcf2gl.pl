...
my $out = "filtered2x_tchum_mel_gbs_2019.gl";
...
open (OUT3, "> dp_$out") or die "Could not write the depth file.\n";
...
foreach my $in (@ARGV){
	...
	while (<IN>){
		...
		elsif ((m/^ScRvNkF_([0-9\.]+):LG\-(\d+):\S+\s+(\d+)/) && (!m/[AGCT],[AGCT]/)){
			$word = "$2:"."$1".":$3";
			if (m/AF1=([0-9\.e]+);/){
				$palt = $1;
				if ($palt > 0.5){
				...
				if ($p >= $maf){ ## keep this locus
					...
					foreach $word (@line){
						if ($word =~ s/^\d\/\d\://){
							$word =~ /(\d+),(\d+),(\d+):(\d+)/ or die "failed match $word \n";
							print OUT " $1 $2 $3";
							print OUT3 "$4 ";
